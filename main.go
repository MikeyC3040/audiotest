...
import (
	...
	"github.com/mikeyc3040/audiotest/gst"
...
<html lang="en">
	...
	<body id="body">
		...
		<script>
			...
			conn.onopen = () => {
				pc.createOffer({offerToReceiveVideo: false, offerToReceiveAudio: true}).then(offer => {
...
func main() {
	httpListenAddress := ""
	flag.StringVar(&httpListenAddress, "http-listen-address", ":8181", "address for HTTP server to listen on")
	flag.Parse()
	...
	audioTrack, err = webrtc.NewTrackLocalStaticSample(webrtc.DefaultPayloadTypeOpus, 5001, "synced-video", "synced-video")
	if err != nil {
		log.Fatal(err)
	}
	...
	pipeline = gst.CreatePipeline(audioTrack)
	pipeline.Start()
